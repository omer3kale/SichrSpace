...
19. [No-Regrets Hardening Roadmap](#no-regrets-hardening-roadmap--sichrspace-backend--platform)
...
### 1.2 CSRF Protection — ~~N/A by design~~

| Item | Detail |
|------|--------|
| Decision | **Not implemented — closed by design** |
| Rationale | Backend is a pure `@RestController` JSON API with `SessionCreationPolicy.STATELESS`. Authentication uses `Authorization: Bearer <JWT>` headers exclusively. Browsers cannot forge cross-site requests with custom headers; no ambient cookie credentials exist. CSRF tokens would be dead code in this architecture. |
| Test coverage | `SecurityConfigTest` — CSRF disabled (no 403 without token); no `Set-Cookie` on auth responses. |
| Future condition | If a Spring MVC / Thymeleaf view layer is ever added that handles `<form>` POST submissions, CSRF must be re-evaluated before those endpoints are deployed. |
| Reference | `SecurityConfig.java` (csrf.disable() comment block) · `docs/SECURITY_AND_SECRETS.md` §10 |
...
### 1.4 WebSocket Realtime (STOMP over WebSocket) — ✅ Complete

| Item | Detail |
|------|--------|
| Previous state | No WebSocket in Spring Boot — realtime was Supabase-only |
| Implemented | `@EnableWebSocketMessageBroker` with SockJS fallback; `JwtChannelInterceptor` validates JWT on every STOMP `CONNECT` frame before the session is established |
| Scope | This is the **primary** realtime backbone — Firebase (Phase 5) will be the optional add-on, not the other way around |
| Endpoint | `/ws` — SockJS handshake; clients connect via `StompClient.connect({Authorization: 'Bearer <jwt>'})` |
| Topics | `/topic/conversations.{id}` (messages), `/user/queue/notifications` (per-user push), `/user/queue/viewing-requests` (status updates) |
| Services wired | `ConversationServiceImpl.sendMessage()`, `NotificationServiceImpl.createNotification()`, `ViewingRequestServiceImpl.confirm/declineViewingRequest()` |
| Injection safety | `SimpMessagingTemplate` injected `@Autowired(required = false)` — null-checked; Mockito unit tests for 3 services remain unaffected |
| New Flyway | None |
| Tests | `JwtChannelInterceptorTest` (5 Mockito tests), `WebSocketConfigTest` (4 SpringBootTest tests) |
| Coverage | Service-level push paths are test-covered via `ReflectionTestUtils` mock injection (COCO service ≥99%, overall 83.1%) |
...
- [x] CSRF: N/A by design — stateless JWT API uses `Authorization: Bearer` headers only; no server-rendered forms exist (Phase 1.2 closed by design)
- [x] JWT secret rotation grace period implemented (`jwtSecretPrevious` fallback in `JwtTokenProvider`)
- [x] WebSocket STOMP infrastructure in place: `WebSocketConfig`, `JwtChannelInterceptor`, push events wired into 3 service impls (9 tests in `JwtChannelInterceptorTest` + `WebSocketConfigTest`)
- [x] Account lockout after 5 failures (4 tests in `AccountLockoutTest`)
- [x] All Phase 1 tests green, COCO gates green (~441 tests, 0 failures)
...
*End of original ROADMAP.md — SichrPlace FTL v1.0.0*  
*Next review: after Backend Lock-In Sprint exit criteria confirmed.*

---
---

# "No-Regrets" Hardening Roadmap — SichrSpace Backend & Platform

> **Added:** 2026-02-23 (Post Phases 1–3 completion)  
> **Baseline:** 643 tests · 85 endpoints · 14 controllers · 86.3 % COCO  
> **Architecture docs:** `docs/architecture/` (domains, modules, flows)

## Premise

- Backend Phases 1–3 are feature-complete (viewings, conversations, dual payments,
  booking-aware transitions) with 643 tests and strong coverage.
- Architecture docs now describe the system at a high level (domains, modules, flows).
- Historical bugs (login failures, broken filters, misaligned viewing/booking UX,
  dead links, confusing promises like "smart matching") show the platform can drift
  away from the intended product.
- This roadmap focuses on **making that drift structurally hard**, mainly via backend
  contracts, ops, and test coverage.

## Guiding Principles

1. **Canonical flows:** Login, profile, search, booking, viewing, payment, and contract
   each have one canonical backend flow; UI can vary, but backend cannot.
2. **Source of truth is the backend:** Filters, labels, and promises on the main page
   must be backed by explicit feature flags + API behaviour.
3. **Bugs become tests and architecture decisions:** Every "ex-bug" is turned into
   either a specific endpoint rule, or a monitoring/config constraint with tests.

---

## Track 1 — Canonical Flows & Contracts (Edge Cases First)

### T1.1 Canonical BookingRequest vs ViewingRequest contracts

> *Goal: No more confusion between "booking" (tenant↔landlord) and "viewing request"
> (tenant↔company).*

**Backend:**
- Lock in two separate aggregate roots:
  - **BookingRequest** (tenant → landlord, lives with apartment domain).
  - **ViewingRequest** (tenant → company, lives with viewing domain).
- Add a short "Contract & Flow" section to the architecture docs mapping:
  - BookingRequest: which endpoints, which statuses, which dashboards.
  - ViewingRequest: which endpoints, which statuses, which dashboards.

**Validation:**
- A booking request **may not** trigger viewing-side email flows or payment flows;
  viewing requests **may not** be visible in landlord dashboards.

**Tests:**
- Negative tests:
  - Cannot submit viewing request through booking endpoints.
  - Cannot see viewing requests in landlord "booking requests" API.

**Ops:**
- Logging field `request_type = BOOKING|VIEWING` in every relevant log record to
  allow dashboards to detect misuse.

### T1.2 Canonical End-to-End "How it works" flow

> *Goal: The instructions become an enforceable contract, not just marketing copy.*

**Backend:**
- For each step in the instruction list, map to concrete domain events/endpoints:
  - Register → Create profile → Search/Post → Chat → Request to book →
    Booking acceptance → Apply for viewing → Confirm → Pay → Receive video →
    Contract → Sign → Move in

- Add to architecture docs a **"Golden Path"** diagram with these steps and the
  events linking them.

**QA:**
- E2E test scenario that follows exactly this canonical flow and asserts:
  - No "Network error" or unexplained 500s at any step.
  - Booking vs viewing vs payment vs contract are all hit in the right order.

**Ops:**
- Canary script or smoke test to run the "short" version of this flow in staging
  before releases.

---

## Track 2 — Identity, Login & Registration Stability

### T2.1 Fix registration and login robustness (renters + landlords)

**Backend:**
- Harmonize registration flows:
  - Same validation rules for email, password, and verification across renter and landlord.
  - Distinct roles recorded in user model (no role-guessing at login).
- Explicit error codes:
  - `EMAIL_NOT_VERIFIED`, `CODE_INVALID`, `CODE_EXPIRED`, `PASSWORD_INCORRECT`, `ACCOUNT_LOCKED`.

**Architecture docs:**
- "Identity & Roles" section: what a renter is, what a landlord is, how session/JWT encodes role.

**Tests:**
- System-level tests reproducing the old issues:
  - "Network error on create account" → controlled 4xx with reason.
  - "Login failed after verification with same email+code".

**Ops:**
- Login/register error dashboards showing rate and breakdown by error code.
- Alert if error rate for registration/login exceeds threshold.

**Status vs current backend:** ⚠️ Mostly done — email verification, password reset,
rate limiting, failed-login tracking, and DB error codes (`USER_EMAIL_ALREADY_EXISTS`,
`ACCOUNT_LOCKED`, etc.) are all implemented in Phases 1–3. Remaining gap: landlord vs
renter distinct registration; explicit `EMAIL_NOT_VERIFIED` / `CODE_EXPIRED` codes.

### T2.2 Forgot password & language-selector bugs

**Backend:**
- Implement fully functional password reset with:
  - One-time token; token expiry; rate limiting.
- Make language selection purely:
  - URL-based (`?lang=de`) or stored in a non-auth cookie; must not intersect with auth/session logic.

**Tests:**
- Password reset success and failure paths.
- Language switch does not break session or cause buttons to disappear.

**Architecture docs:**
- Document language selector as "cosmetic" (influencing text only), not a stateful backend feature.

**Status vs current backend:** ✅ Password reset is fully implemented (forgot-password →
reset-password flow with token expiry). Language selector is frontend-only; no backend
impact. Tests exist for password reset happy/failure paths.

---

## Track 3 — Filters, Listings, and Profile Data Alignment

### T3.1 Align search filters with apartment model

> *Goal: Fix all "missing filter" / "duplicate advanced filter" / "filter button does
> not work" issues at the root.*

**Backend:**
- Treat the apartment search schema as the canonical contract:
  - **Basic:** city/area, move-in/move-out, earliest move-in, flexible vs fixed slot,
    price (kalt/warm), property type (shared room, private room, studio, loft, apartment,
    house), room/beds, furnished.
  - **Advanced:** amenities + extra constraints (exclude exchanges, pets yes/no).
- Ensure apartment entity + search endpoint support all of these fields and no "mystery" ones.

**Tests:**
- For each documented filter: integration test showing it actually influences results.
- Regression test: filter button triggers backend search; no no-op.
- Regression test: advanced filter set appears once in API (not duplicated).

**Ops:**
- Log filter usage; if frontend sends unknown filter keys or wrong types, log structured warnings.

### T3.2 Applicant profile fields & landlord visibility

**Backend:**
- Canonical profile model includes:
  - Habits, daily routines, lifestyle tags, maybe gender (if legally/compliantly handled),
    but hides highly sensitive data from landlords.
- Add "Limited profile view" API: `GET /api/profiles/{id}/public` (landlord-safe subset).

**Tests:**
- Personality/habits questions exist in profile API.
- Applicant dashboard does not expose landlord-only functions (tenant screening, contract generation).
- Landlord cannot see raw sensitive data.

---

## Track 4 — Viewing & Booking UX Edge Cases (Backend Guardrails)

### T4.1 Viewing request form constraints

**Backend:**
- Enforce that ViewingRequest API:
  - Does **NOT** accept monthly budget range or "additional guests" fields.
  - Does accept: `apartmentId`, `date`, `time`, `questions`, `whatToPayAttentionTo`.
- Add metadata to ViewingRequest endpoints indicating:
  - "We, the company, perform the viewing" (for future UI hints).

**Tests:**
- Reject unsupported fields at schema level.
- Confirm time and date requirements; enforce pre-agreed landlord slot if applicable.

**Status vs current backend:** ⚠️ ViewingRequest exists with `proposedDateTime` and
`message` fields. Needs explicit **rejection** of budget/guest fields + metadata flag.

### T4.2 BookingRequest form (Wunderflats-style) and comparison/matching

**Backend:**
- Enforce BookingRequest structure:
  - Stay duration (move-in/out, extend later).
  - Tenants: names, relationships, child ages, pets.
  - Reason (work/study/temporary etc.), payer.
- Extend matching logic:
  - Support grouping applicants to multi-room apartments (matching best-fitting
    two/three applicants to 2–3 free rooms).

**Tests:**
- Incomplete BookingRequests return proper 4xx with clear messages.
- Matching logic: when there are N free rooms and multiple applicants, API returns
  recommended groupings.

**Status vs current backend:** 🆕 BookingRequest is entirely new. This is the highest-priority
new domain entity.

---

## Track 5 — Content, Navigation & Promises vs Reality

### T5.1 Static nav and footer links

> *Goal: No "About/FAQ/Customer Service/Scam Stories: page not found" any more.*

**Backend/docs:**
- Every link present in nav/footer maps to:
  - Either a real content endpoint, or
  - A deliberate "coming soon" stub with proper JSON and `501 Not Implemented` / `204 No Content`.
- For "Scam stories" and "Customer Service contact form":
  - Expose backend endpoints callable from the UI even if content is minimal.

**Tests:**
- Navigation API tests verifying all known routes respond successfully.

### T5.2 "Smart matching" and "secure payments" checks

**Backend:**
- Implement feature flags:
  - `smart_matching_enabled`, `secure_payments_enabled`.
- Main-page API must return booleans for these features.

**Ops:**
- If feature is off, the UI must not show "Smart matching" or else log an error.

**Tests:**
- Feature flag API correctly reflects actual backend functionality.

**Status vs current backend:** `secure_payments_enabled` can be set to `true` (Stripe + PayPal
are live). `smart_matching_enabled` remains `false` until matching algorithm is implemented.

### T5.3 Customer service and support flows

**Backend:**
- Decide: "customer service page form → email" vs "internal ticket".
- Document explicitly in architecture docs.
- Ensure both main-page support form and dedicated customer service page send messages
  through the same backend pipeline.

**Tests:**
- Support form on main page hits a real endpoint.
- Dedicated customer service page form hits the same endpoint.
- No form leads to a dead endpoint.

---

## Track 6 — Admin, Ops & Observability

### T6.1 Admin dashboard APIs

**Backend:**
- Implement clear admin endpoints for:
  - Listings (with statuses).
  - Users (verification, activity).
  - Viewing requests (company-side).
  - Booking requests (landlord-side).
  - Payments (logs, disputes, refunds).
- Ensure admin actions match dashboard ideas (activate/deactivate offers, view checks,
  assign account reps).

**Tests:**
- Admin API tests with role-based access controls.
- Regression test: landlord dashboard sees BookingRequests but not ViewingRequests.

**Status vs current backend:** ⚠️ `AdminController` exists with user management +
listing moderation + conversation report endpoints. Missing: booking request admin,
payment refund admin, account rep assignment.

### T6.2 Payments & contracts ops

**Backend:**
- Stripe/PayPal webhooks are idempotent.
- All contract creation steps are auditable (who clicked what, when).
- Document where contracts are stored; how feedback emails (day of move-in, 1 month,
  6 months, end) are scheduled.

**Ops:**
- Alerts for webhook failures and contract generation failures.
- Logs correlate `payment → viewing → contract`.

**Status vs current backend:** ⚠️ Webhooks exist and work but lack idempotency keys
(Post-Phase 3 backlog item B-7). Contract creation is not yet implemented.

### T6.3 Health checks, bug dashboards, and regression harness

**Backend:**
- Health endpoints for: DB, payment providers, mail provider.
- "Bug regression suite": each ex-bug becomes a short integration/E2E scenario.

**Ops:**
- Dashboards for:
  - Login/registration error rates.
  - Filter usage and error rates.
  - Viewing/booking flow completion rates.

**QA:**
- Run regression harness before each release.

**Status vs current backend:** ✅ `GET /api/health` and `GET /api/health/db-readiness`
exist with tests. Payment and mail provider health checks are new.

---

## Track Priority & Dependencies

> **Revised 2026-02-23:** BookingRequest (T1.1, T4.2) is parked as Phase 5 future work.
> The hardening sprints focus exclusively on the existing ViewingRequest + payments +
> filters + auth system. No new domain entities until the current experience is solid.

```
HARDENING SCOPE (existing system — do now)
──────────────────────────────────────────────
Track 2 (Identity)
  └─► T2.1 Registration robustness           ← ~80% done, incremental — SPRINT 1
  └─► T2.2 Password reset + locale           ← ✅ done

Track 4 (Viewing Guardrails — existing ViewingRequest only)
  └─► T4.1 Viewing form constraints          ← independent of BookingRequest — SPRINT 1

Track 3 (Filters & Profiles)
  └─► T3.1 Search schema alignment           ← align with current apartment model — SPRINT 2
  └─► T3.2 Profile visibility                ← independent — SPRINT 4

Track 5 (Content & Promises)
  └─► T5.1 Nav/footer links                  ← independent — SPRINT 2
  └─► T5.2 Feature flags                     ← independent — SPRINT 2
  └─► T5.3 Customer service                  ← independent — SPRINT 4

Track 6 (Admin & Ops — existing ViewingRequest + payments only)
  └─► T6.1 Admin APIs (viewing + payment)    ← no BookingRequest admin yet — SPRINT 3
  └─► T6.2 Payment ops (idempotency)         ← Phase 3 payments done — SPRINT 3
  └─► T6.3 Health + regression               ← independent — SPRINT 3

Track 1 (Canonical Flows — ViewingRequest-scoped)
  └─► T1.2 Golden Path E2E (ViewingRequest)  ← after hardening sprints — SPRINT 4

FUTURE (Phase 5 — domain expansion, not current scope)
──────────────────────────────────────────────────────────
Track 1 (Canonical Flows)
  └─► T1.1 BookingRequest domain             ← new entity, Wunderflats-style form

Track 4 (Booking Guardrails)
  └─► T4.2 BookingRequest form + matching    ← depends on T1.1

Track 6 (Admin)
  └─► T6.1b BookingRequest admin APIs        ← depends on T1.1
```

## Implementation Order (Revised — Hardening Existing System)

| Sprint | Tracks | Key Deliverable |
|--------|--------|-----------------|
| **Sprint 1** | T2.1 + T4.1 | Registration error codes (`EMAIL_NOT_VERIFIED`, `CODE_EXPIRED`), ViewingRequest field rejection (no budget/guests) |
| **Sprint 2** | T3.1 + T5.1 + T5.2 | Search schema alignment with apartment model, nav/footer stubs, feature flags (`smart_matching_enabled`, `secure_payments_enabled`) |
| **Sprint 3** | T6.1 + T6.2 + T6.3 | Admin viewing/payment APIs, webhook idempotency keys, health checks, regression harness |
| **Sprint 4** | T1.2 + T3.2 + T5.3 | Golden Path E2E (ViewingRequest flow only), profile visibility, customer service endpoint |
| **Phase 5** | T1.1 + T4.2 + T6.1b | BookingRequest domain — **future work, not current scope** |

---

## Cross-Reference: Hardening Tracks vs Original Phases

| Hardening Track | Maps to Original Phase | Scope | Sprint |
|-----------------|----------------------|-------|--------|
| T2.1 Registration | Phase 0–1 (Foundation + Auth) | **HARDEN NOW** | 1 |
| T4.1 Viewing guardrails | Phase 2 (Payments scope) | **HARDEN NOW** | 1 |
| T3.1 Search schema | Current apartment model | **HARDEN NOW** | 2 |
| T5.1 Nav stubs | Phase 11 (Frontend wiring) | **HARDEN NOW** | 2 |
| T5.2 Feature flags | Ops / truthfulness | **HARDEN NOW** | 2 |
| T6.1 Admin APIs (viewing/payment) | Phase 0–1 existing admin | **HARDEN NOW** | 3 |
| T6.2 Payment ops | Phase 2 (Payments) + backlog B-7 | **HARDEN NOW** | 3 |
| T6.3 Health + regression | Phase 8 (Integration Tests) | **HARDEN NOW** | 3 |
| T1.2 Golden Path E2E | Phase 8 (Integration Tests) | **HARDEN NOW** | 4 |
| T3.2 Profile visibility | Phase 11 (Frontend wiring) | **HARDEN NOW** | 4 |
| T5.3 Customer service | Phase 11 (Frontend wiring) | **HARDEN NOW** | 4 |
| T2.2 Password reset | Phase 0 (Foundation) | ✅ Done | — |
| T1.1 BookingRequest | New domain (Wunderflats-style) | **PHASE 5 FUTURE** | — |
| T4.2 BookingRequest form | Same as T1.1 | **PHASE 5 FUTURE** | — |

---

*End of "No-Regrets" Hardening Roadmap — revised 2026-02-23.*  
*BookingRequest (T1.1, T4.2) explicitly parked as Phase 5 future work.*

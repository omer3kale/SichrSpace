...
## FTL Final Build Summary

> **Date:** 2026-02-23  
> **Tests:** 807 · **Failures:** 0 · **Coverage:** 88.8% overall · **COCO:** All gates GREEN

- All golden-path steps 1–12 are under test (happy-path + error coverage)
- Step 13 (contract/move-in) is explicitly deferred — feature not yet built
- Turkish locale hardening applied across 14 call sites (`Locale.ROOT`)
- Service coverage: 99.2% · Controller coverage: 99.4%
- 89 endpoints across 15 controllers verified
- Security layer: 100% coverage — JWT, rate-limiting, account-locking all tested

**Regression baseline:** Any future change must pass `./gradlew clean testWithCoverage checkCoco` with 0 failures.

---

## FTL v2 — Extended Golden Paths (2026-02-24)

### GP-13: Review After Completing a Viewing

```
1. Tenant requests viewing           → POST /api/viewing-requests
2. Landlord confirms                  → PUT  /api/viewing-requests/{id}/confirm
3. Either party completes             → PUT  /api/viewing-requests/{id}/complete
4. System sends review-prompt email   → (automatic via NotificationService)
5. Tenant submits review              → POST /api/reviews/apartment/{id}
   ├── Must have COMPLETED viewing or ACCEPTED booking
   └── Rejected if no qualifying viewing/booking exists
6. Review enters moderation           → GET  /api/admin/reviews/pending
7. Admin approves                     → POST /api/admin/reviews/{id}/moderate
8. Landlord stats update              → GET  /api/reviews/landlord/{landlordId}
```

### GP-14: Viewing Credits (3-Pack Behavior)

```
1. First paid viewing   → Payment succeeds → ViewingCreditPack created (3 credits, 1 used)
2. Second viewing       → Active pack found → useCredit() → 1 remaining (FREE)
3. Third viewing        → Active pack found → useCredit() → 0 remaining (FREE)
4. Fourth viewing       → No active pack    → New payment required → New 3-credit pack
5. Check credit status  → GET /api/viewing-credits/me/has-credit → true/false
6. View history         → GET /api/viewing-credits/me → full summary
```

### GP-15: Support Ticket Flow

```
1. User creates ticket            → POST /api/support/tickets (201)
   ├── System sends SUPPORT_TICKET_CREATED notification
   └── Category defaults to GENERAL if omitted
2. User views own tickets         → GET  /api/support/tickets
3. Admin lists all tickets        → GET  /api/admin/support/tickets?status=OPEN
4. Admin responds                 → PATCH /api/admin/support/tickets/{id}
   ├── Blank response → 400
   ├── Status defaults to RESOLVED
   └── Sends SUPPORT_TICKET_UPDATED notification + email
```

### GP-16: Search Filters + Locale Validation

```
1. Search with new filters  → GET /api/apartments?propertyType=WG&hasWifi=true&availableFrom=2026-03-01
   ├── Invalid propertyType silently skipped (no error)
   └── Amenity booleans: hasWifi, hasWashingMachine, hasDishwasher, hasAirConditioning, hasHeating
2. Update locale            → PUT /api/auth/profile { "preferredLocale": "de" }
   ├── Valid: en, de, tr (case-insensitive, normalized to lowercase)
   └── Invalid (e.g., "fr") → IllegalArgumentException
3. DTO validation           → @Pattern(regexp="^(en|de|tr)$") blocks invalid values at deserialization
```

### GP-17: Smart Matching for Landlords

```
1. Landlord compares applicants → GET /api/matching/applicants/{apartmentId}
   └── Returns ranked list with scores + reasons + public profiles
2. Check success rate           → GET /api/matching/success-rate
   └── Returns totalBookingRequests, acceptedBookings, successRate (0.0–1.0)
3. Payment health               → GET /api/health/payments
   └── Reports Stripe + PayPal configuration status (UP/DEGRADED)
```

---

## FTL v2 Final Build Summary

> **Date:** 2026-02-24
> **Tests:** 862 · **Failures:** 0 · **Coverage:** 88.9% overall · **COCO:** All gates GREEN
> **Delta:** +55 tests · +20 endpoints · +7 controllers with new code

- controller: 99.2% · service: 99.1% · security: 100%
- 109 endpoints across 22 controllers verified
- New features: ViewingCredits, SupportTickets, Landlord Reviews, Smart Matching (landlord), Search Filters, Locale Validation
- All golden-path steps GP-13 through GP-17 covered by unit tests

**Regression baseline:** `.\gradlew.bat clean testWithCoverage checkCoco --no-daemon --console=plain` — 862 tests, 0 failures.
